...
The \gls{ci} cycle is primarily intended for teams of developers. \gls{ci} leads to a `Test Early. Test Often. Test Automatically.' attitude --- it drives the developer to find find bugs early, which helps to avoid users finding bugs later and increases confidence in the system \cite{pragmaticProgrammer}.  Risks associated with deferred integration (integration after a long time or large numbers of changes, antonymous to continuous integration) include the risk of bugs being released as part of a product.
...
A factor which was not taken into when considering \gls{ci} was the cost in time of setting up the environment in the first place and the possibility of the server failing.  The time spent on introducing \gls{ci} was not recouped in time saved by identifying problems and, particularly in a one-developer project such as this, it is not recommended as an effective aid to development.
